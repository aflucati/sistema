...
export interface ValidationMismatch {
  key: string;
  issue: 'missing_in_engine' | 'extra_in_engine' | 'value_mismatch';
  expected?: Record<string, string | number>;
  actual?: Record<string, string | number>;
}

function normalizeExpectedSplit<T extends { prazoCd: number; prazoTr: number; prazoCliente: number }>(
  row: T,
): T {
...
function toComparableRecord(row: ValidatedRow): Record<string, string | number> {
...
function toCanonicalDay(value: string): string {
...
function addBusinessDays(startDayNumber: number, businessDays: number): number {
  if (businessDays <= 0) {
...
function inferFrequencyFromRows(rows: ValidatedRow[]): string {
  ...
  const candidates: Array<{ frequency: string; extraDays: number }> = [
...
function inferOfferedDayFromRows(rows: ValidatedRow[]): string {
  const referenceRow = [...rows].sort((left, right) => left.prazoCd - right.prazoCd)[0];
  const chargeDayIndex = DAY_INDEX_MAP[referenceRow.productionDay];
  ...
  const inferredLabel = ['Segunda', 'Terca', 'Quarta', 'Quinta', 'Sexta', 'Sabado', 'Domingo'][
    ((inferredOfferedDayNumber % 7) + 7) % 7
  ];

  return inferredLabel ?? referenceRow.offeredDay;
}

export function loadValidatedRows(csvPath: string): ValidatedRow[] {
...
export function buildRoutesFromValidatedRows(rows: ValidatedRow[]): RouteInput[] {
  const grouped = new Map<string, RouteInput>();
  const frequencyByEventKey = new Map<string, string>();
  const offeredDayByEventKey = new Map<string, string>();

  const rowsByEventKey = new Map<string, ValidatedRow[]>();
  ...
  for (const row of rows) {
    ...
    const exists = route.events.some(
      (event) =>
        `${toCanonicalDay(event.day)}|||${String(event.cutoffHour)}|||${toCanonicalDay(event.deliveryDay)}` ===
        eventKey,
    );
...
function expandEngineRows(rows: WindowRow[]): Array<Record<string, string | number>> {
  const expanded: Array<Record<string, string | number>> = [];
  const saleDayByKey: Array<{ key: keyof WindowRow; label: string }> = [
...
export function compareWithValidatedRows(validatedRows: ValidatedRow[]): ValidationMismatch[] {
  const routes = buildRoutesFromValidatedRows(validatedRows);
  const engineRows = expandEngineRows(calculateDeadlines(routes).rows);
  const mismatches: ValidationMismatch[] = [];

  const actualRowsByRouteDay = new Map<string, Array<Record<string, string | number>>>();
  ...
  for (const expectedRow of validatedRows) {
    ...
    const matchingCandidate = candidates.find(
      (actual) =>
        Number(actual.prazoCd) === normalizedExpected.prazoCd &&
    ...
    const closestCandidate = [...candidates].sort(
      (left, right) => Number(left.startHour) - Number(right.startHour),
    )[0];
