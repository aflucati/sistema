...
exports.calculateDeadlines = calculateDeadlines;
...
const DEFAULT_TEMPLATE_PATH = path_1.default.join(resolveAssetsDirectory(), 'template_oficial_horizontal.html');
const DAY_ALIASES = {
    segunda: 0,
    'segunda-feira': 0,
    terca: 1,
...
function parseDeliveryRule(value) {
    const trimmed = String(value).trim();
...
function normalizeCd(commercialLocation, explicitCd) {
...
function deriveLocality(commercialLocation, locality, routeDestination) {
    if (locality?.trim()) {
...
function calculateDeliveryDayNumber(chargeDayNumber, deliveryDayIndex, deliveryOffsetDays) {
...
function dayNumberToLabel(dayNumber) {
...
function createBaseRow(event, startHour, endHour, orderDayNumber, cargasPorSemana, selectedEvent, selectedTimestamp) {
    const orderDayIndex = ((orderDayNumber % 7) + 7) % 7;
    const chargeDayNumber = Math.floor(selectedTimestamp / 24);
    let realDeliveryDayNumber = calculateDeliveryDayNumber(chargeDayNumber, selectedEvent.deliveryDayIndex, selectedEvent.deliveryOffsetDays);
...
function buildRouteWindows(events) {
    ...
    for (let index = 0; index < orderedEvents.length; index += 1) {
        ...
        const intervalStart = previousEvent.timestamp >= selectedEvent.timestamp
        ...
        for (let orderDayNumber = startDayNumber; orderDayNumber <= endDayNumber; orderDayNumber += 1) {
            ...
            rows.push(createBaseRow(events[0], startHour, endHour, orderDayNumber, cargasPorSemana, selectedEvent, selectedEvent.timestamp));
        }
    }
...
function buildHtml(rows) {
    const template = fs_1.default.readFileSync(DEFAULT_TEMPLATE_PATH, 'utf8');
    const htmlRows = rows
        .map((row) => {
        const cells = [
            row.cd,
        ...
        const dayCells = DAY_KEYS.map((dayKey) => {
            ...
            return `<td class="ok" title="${escapeHtml(details)}" onclick="showModal('${escapeJsString(details)}')" style="cursor:pointer">OK</td>`;
        }).join('');
        return `<tr>${staticCells}${dayCells}</tr>`;
    })
        .join('\n');
    return template.replace(/<!--DATA_ROWS_START-->[\s\S]*<!--DATA_ROWS_END-->/, `<!--DATA_ROWS_START-->\n${htmlRows}\n<!--DATA_ROWS_END-->`);
}
function expandRowsByModal(rows) {
    return rows.flatMap((row) => splitModalValues(row.modal).map((modal) => ({
        ...row,
        modal,
